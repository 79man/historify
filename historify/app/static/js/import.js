...
function selectImportMethod(method) {
  ...
  affected_sections = [
    ...
    "paste-ohlc-import-section",
  ];
  affected_sections.forEach((section) =>
    document.getElementById(section).classList.add("hidden")
  );

  // Show selected section
  document
    .getElementById(`${method}-import-section`)
    ...
    fileItem.innerHTML = `
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-sm text-muted">${formatFileSize(
                      file.size
                    )}</p>
                </div>
            </div>
...
function displayPreview(data) {
  ...
  if (data.length > 11) {
    ...
    tr.innerHTML = `<td colspan="${
      data[0].length
    }" class="text-center text-muted">... and ${
      data.length - 11
    } more rows</td>`;
    previewBody.appendChild(tr);
  }
...
async function validateImportData() {
  ...
  for (const file of importData.files) {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      const exchange = exchangeCol
        ? row[exchangeCol]?.trim().toUpperCase()
        : defaultExchange;
      ...
      if (symbol) {
        ...
        const isDuplicate = importData.symbols.some(
...
function displayValidationResults() {
  ...
  if (importData.invalidSymbols.length > 0) {
    ...
    section.innerHTML =
      '<h4 class="font-medium mb-2 text-error">Invalid Symbols:</h4>';

    importData.invalidSymbols.forEach((item) => {
      const div = document.createElement("div");
      div.className =
        "flex items-center gap-2 text-sm p-2 bg-error/10 rounded mb-1";
      div.innerHTML = `
  ...
  if (importData.duplicateSymbols.length > 0) {
    ...
    section.className = "mb-4";
    section.innerHTML =
      '<h4 class="font-medium mb-2 text-warning">Duplicate Symbols:</h4>';

    importData.duplicateSymbols.forEach((item) => {
      const div = document.createElement("div");
      div.className =
        "flex items-center gap-2 text-sm p-2 bg-warning/10 rounded mb-1";
      div.innerHTML = `
...
function prepareBatchData(batch, data_type, metadata) {
  if (data_type === "ohlc" && metadata) {
    // Return OHLC-specific format
    return {
      symbol: metadata.symbol,
      exchange: metadata.exchange,
      interval: metadata.interval,
      csv_data: batch, // batch is already the slice of csvData
    };
  } else {
...
function getImportEndpoint(data_type) {
  return data_type == "ohlc" ? "/api/import-ohlc-data" : "/api/import-symbols";
}
...
async function processImport(
  ...
  const validData = dataToProcess || importData.validSymbols;
  ...
  setTimeout(() => {
    progressModal.querySelector(".modal-backdrop").classList.add("show");
  ...
  let processed = 0;
  const apiEndpoint = getImportEndpoint(data_type);
  ...
  for (const batch of batches) {
    try {
      const response = await fetch(apiEndpoint, {
        ...
        body: JSON.stringify(prepareBatchData(batch, data_type, metadata)),
      });
      ...
      if (response.ok) {
        ...
        const progress = Math.round(
          (processed / validData.length) * 100
        );
...
function validateOHLCData() {
  const pasteInput = document.getElementById("paste-ohlc-input").value.trim();
  const symbol = document.getElementById("ohlc-symbol").value.trim();
  const exchange = document.getElementById("ohlc-exchange").value;
  const interval = document.getElementById("ohlc-interval").value;

  if (!symbol) {
    showToast("Please enter a symbol for OHLC data", "warning");
    return;
  }

  // Parse tab-separated OHLC data
  ...
  if (lines.length === 0) {
    showToast("No valid OHLC data found", "error");
    return;
  }
  ...
  dataLines.forEach((line, index) => {
    try {
      // Split by tab (your format is tab-separated)
      const parts = line.split("\t").map((p) => p.trim());

      if (parts.length >= 6) {
        // Your format: date, close, open, high, low, volume
        csvData.push({
          date: parts[0],
          close: parts[1],
          open: parts[2],
  ...
  if (errorCount > 0) {
    showToast(
      `Found ${errorCount} invalid rows, proceeding with ${csvData.length} valid rows`,
      "warning"
    );
  }

  // Pass the raw data and metadata separately
  const ohlcMetadata = {
    symbol: symbol,
    exchange: exchange,
    interval: interval,
  ...
  processImport("ohlc", csvData, 100, ohlcMetadata);
}

function validateSymbolData() {
  // Existing symbol validation logic from the original function
  const pasteInput = document.getElementById("paste-input").value.trim();
  const format = document.getElementById("paste-format").value;
  const defaultExchange = document.getElementById(
    "paste-default-exchange"
  ).value;
...
function resetImport() {
  ...
  document.getElementById("paste-ohlc-input").value = "";
...
async function loadOHLCSymbols() {
  try {
...
function populateSymbolDropdown(symbols) {
  const dropdown = document.getElementById("ohlc-symbol");

  // Clear existing options except the first one
  dropdown.innerHTML = '<option value="">Select a symbol...</option>';

  // Group symbols by exchange like in export.js
  const grouped = {};
  symbols.forEach((symbol) => {
    const exchange = symbol.exchange || "Unknown";
    if (!grouped[exchange]) {
      grouped[exchange] = [];
    }
  ...
  // Add options grouped by exchange
  Object.entries(grouped).forEach(([exchange, exchangeSymbols]) => {
    ...
    exchangeSymbols.forEach((symbol) => {
      ...
      option.dataset.exchange = symbol.exchange;
...
function handleSymbolSelection() {
  const symbolDropdown = document.getElementById("ohlc-symbol");
  const exchangeDropdown = document.getElementById("ohlc-exchange");

  const selectedOption = symbolDropdown.options[symbolDropdown.selectedIndex];
  if (selectedOption && selectedOption.dataset.exchange) {
    exchangeDropdown.value = selectedOption.dataset.exchange;
  }
}

document.addEventListener("DOMContentLoaded", function () {
  loadOHLCSymbols();
  // Add event listener to the symbol dropdown
  document
    .getElementById("ohlc-symbol")
    .addEventListener("change", handleSymbolSelection);
});
